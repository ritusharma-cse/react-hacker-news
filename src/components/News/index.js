...
class News extends Component {
  ...
  async performUpdationOnStateAfterGettingData({ result = {}, error }) {
    ...
    let upVotes = await hackerNApi.getDataFromStorageServices('upVotes')
    const hidden = await hackerNApi.getDataFromStorageServices('hidden');
    if (upVotes || hidden) {
    ...
    const bookmark = await hackerNApi.getDataFromStorageServices('bookmark');
  ...
  getPageWiseData() {
    ...
    hackerNApi.getPageWiseData(page)
      .then((result) => {
  ...
  onUpVoteClickHandler = async (e, objectID) => {
    ...
    await hackerNApi.setDataToStorageService('upVotes', upVotes);
    let { hits } = this.state;
  ...
  onHideButtonHandler = async (e, objectID, isHide) => {
    e.preventDefault();
    let hidden = await hackerNApi.getDataFromStorageServices('hidden')
    if (!hidden) {
      hidden = {};
    }
    hidden[objectID] = isHide;
    await hackerNApi.setDataToStorageService('hidden', hidden);
    let { hits } = this.state;
    let index = hits.findIndex(element => {
      return element.objectID === objectID;
    });
  ...
  onBookMarkButtonHandler = async (isBookmark) => {
    ...
    let bookmarkObj = await hackerNApi.getDataFromStorageServices('bookmark')
    if (!bookmarkObj) {
      bookmarkObj = {};
    }
    bookmarkObj[page] = isBookmark;

    await hackerNApi.setDataToStorageService('bookmark', bookmarkObj);
  }
